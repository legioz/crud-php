...
<tbody>
    <?php foreach ($result as $key => $value) : ?>
      <tr>
         <td id="<?= $value['aluno']; ?>" ><?= $value['aluno']; ?></td>
         <td id="<?=$value['fk_turma']?>"><?= $value['turma']. " - ".$value['fk_turma'] ; ?></td>
         <td><?= $value['ano'];?></td>
         <td><?= $value['semestre']; ?></td>
         <td>
          <!-- <button id="#btnEditaMatricula" type="submit" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#editaMatricula" onclick="
          editaMatricula(<?= $value['id'];?>, <?= $value['aluno']; ?>)">Editar</button> -->
          <button id="#btnEditaMatricula" type="submit" class="btn-edita btn btn-warning btn-sm" data-toggle="modal">Editar</button>
...
<div class="modal" role="dialog" id="editaMatricula">
    <div class="modal-dialog" role="document">
        ...
        <div class="form-group">
        <label>Código Turma</label>
        <select name="idTurma" id="idTuma" class="form-control">
          <option selected invalid>Escolher...</option>
            <?php 
                foreach ($resultTurma as $key => $value):
                $turma = $value['ano'].".". $value['semestre']." - ". $value['turma']." - ". $value['curso'];
            ?> 
            <option value="<?=$value['id']?>"> <?=$turma?> </option>
            <?php endforeach;?>
        </select>
        </div>
...
<script>
    $(document).ready(function() {
        $(".btn-edita").click(function() {
            var aluno = $(this).closest("tr").find('td').prop("id");
            $("#raAluno").val(aluno);
            console.log(aluno);
            var turma = $(this).closest("tr").find('td').next().attr("id");
            $("#editaMatricula").modal("show");
            $("#idTuma option").each(function() {
                var option = $(this).val();
                if(option == turma) {
